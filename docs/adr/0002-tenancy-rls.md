...
- `ajour_app` — all domain queries. Policies key on
...
- `ajour_auth` — Better Auth's own connection pool, and the admission
  ...
  tables. Conversely `ajour_app` cannot read `accounts`, `sessions`,
...
RLS separates workspaces; it does not separate projects within one. A
person who belongs to a workspace can reach every project in it. Finer
rights inside a workspace, if they are ever needed, are an application
concern on top of this model, never a replacement for it.
...
- `ajour_auth` is trusted with all auth tables; Better Auth's own logic is
...
- Policies key on the session's _active_ workspace; a person with several
  ...
  as a security one.
- Share links (wave 2) are the one deliberate way to read a project without
  a session. They will run through a dedicated, minimal query with the
  workspace context set by the token's own row, never through a bypass
  role.
