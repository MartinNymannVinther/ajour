# ADR 0009: A workspace can choose its own model
...
edit tasks has no business changing what the workspace spends or which
company reads its project text.
...
`workspaceLlmProvider()` instead, which means `chosenEngine()` and
`withEngine()` now take an `OrgContext`. The rules engine is still what
answers when nothing else can, and a workspace that sets its provider to
"none" gets it deliberately, even on an installation with a model
configured.
