...
One-off tools, run by hand against an Ajour database. None of them is
...
no "forgot password" to fall back on, because Ajour has no email provider
yet.
...
VPS from cron, not in a container; docs/deploy.md section 6 has the setup.
