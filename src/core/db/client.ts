 ...
 *
 * - appPool/appDb  → role `ajour_app`: every domain query. Fully subject to
 *   RLS; without an org context set it sees nothing (default deny).
 * - authPool/authDb → role `ajour_auth`: used only by Better Auth and the
...
export const appPool =
  globalForDb.ajourAppPool ?? new Pool({ connectionString: env.APP_DATABASE_URL, max: 10 });

export const authPool =
  globalForDb.ajourAuthPool ?? new Pool({ connectionString: env.AUTH_DATABASE_URL, max: 10 });

if (env.NODE_ENV !== "production") {
  globalForDb.ajourAppPool = appPool;
  globalForDb.ajourAuthPool = authPool;
}
