...
const version = process.env.AJOUR_VERSION || "0.0.0";
const commit = process.env.AJOUR_COMMIT || "unknown";

export const buildInfo: BuildInfo = {
  version,
  commit,
  dirty: process.env.AJOUR_DIRTY === "1",
  builtAt: process.env.AJOUR_BUILT_AT || "",
  release: formatRelease(version, commit),
  migration: process.env.AJOUR_MIGRATION || "unknown",
  migrationCount: Number(process.env.AJOUR_MIGRATION_COUNT || "0"),
};
