...
export async function createDemoWorkspace(locale: "da" | "en"): Promise<DemoSession> {
  ...
  const projectId = await withOrgContext({ orgId: organization.id, userId }, (tx) =>
    seedDemoProject(tx, { orgId: organization.id, userId }, locale),
  );

  return { headers: sessionHeaders, projectId };
}
