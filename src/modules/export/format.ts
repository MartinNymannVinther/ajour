 ...
 * openable by a board member who has never heard of Ajour. The JSON is
...
export function exportFileName(orgName: string, exportedAt: Date, extension: string): string {
  ...
  return `ajour-${slug}-${date}.${extension}`;
}
...
export function exportToXlsx(data: OrgExport): Buffer {
  const sheets: Sheet[] = data.sections.map((section) => ({
    name: section.sheet,
    // An empty table still gets its tab, so the file shows what Ajour holds
...
export function exportToJson(data: OrgExport): string {
  ...
  return JSON.stringify(
    {
      ajour: { format: 1, exportedAt: data.exportedAt.toISOString() },
