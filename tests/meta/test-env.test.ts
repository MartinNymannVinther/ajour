...
const dev = {
  MIGRATION_DATABASE_URL: "postgres://postgres:postgres@localhost:5432/ajour",
  APP_DATABASE_URL: "postgres://ajour_app:ajour_app@localhost:5432/ajour",
  AUTH_DATABASE_URL: "postgres://ajour_auth:ajour_auth@localhost:5432/ajour",
};
...
describe("resolveTestDatabaseUrls", () => {
  it("sends an ordinary .env to the _test sibling, keeping server and credentials", () => {
    ...
    expect(databaseName(resolved.MIGRATION_DATABASE_URL)).toBe("ajour_test");
    expect(databaseName(resolved.APP_DATABASE_URL)).toBe("ajour_test");
    expect(resolved.APP_DATABASE_URL).toContain("ajour_app:ajour_app@localhost:5432");
  });

  it("leaves a name that is already a test database alone", () => {
    const resolved = resolveTestDatabaseUrls({
      ...dev,
      MIGRATION_DATABASE_URL: "postgres://postgres:postgres@localhost:5432/ajour_test",
    });
    expect(databaseName(resolved.MIGRATION_DATABASE_URL)).toBe("ajour_test");
  });
  ...
  it("refuses an override that does not name a test database", () => {
    expect(() =>
      resolveTestDatabaseUrls({
        ...dev,
        TEST_APP_DATABASE_URL: "postgres://ajour_app:ajour_app@localhost:5432/ajour",
      }),
  ...
  it("refuses an override aimed straight at the working database", () => {
    ...
    expect(() =>
      resolveTestDatabaseUrls({
        ...dev,
        APP_DATABASE_URL: "postgres://ajour_app:ajour_app@localhost:5432/ajour_test",
        TEST_APP_DATABASE_URL: "postgres://ajour_app:ajour_app@localhost:5432/ajour_test",
      }),
...
describe("maintenanceUrl", () => {
  it("points at the postgres database on the same server, so the test one can be created", () => {
    expect(maintenanceUrl("postgres://postgres:pw@localhost:5432/ajour_test")).toBe(
